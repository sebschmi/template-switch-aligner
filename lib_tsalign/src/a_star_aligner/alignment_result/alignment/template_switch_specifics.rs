...
impl Alignment<AlignmentType> {
    ...
    >(
            ...
            )
        {
            ...
            .and_then(|i| usize::try_from(i.checked_add(first_offset)?).ok()) else {
                debug_assert!(
                    false,
                    "Error finding inner secondary index -- integer math does not check out"
                );
                return false;
            };
            ...
            *multiplicity -= 1;
            ...
            // TODO is ts_inner_primary_index and ts_inner_secondary_index always >0?
    ...
    >(
        ...
        {
            debug_assert!(false, "We did not want flanks to get involved...");
            return false;
        }
    ...
    >(
        ...
        let Some((exit_index_offset, _)) =
            ...
            debug_assert!(false, "There should be a TS exit after the TS entrance");
            return false;
        };
            ...
            self.alignment.get(exit_index + 1)
        {
            ...
            *multiplicity -= 1;
            ...
            let secondary_char = match direction {
                ...
                TemplateSwitchDirection::Reverse => {
                    // TODO is ts_inner_secondary_index always >0?
    ...
    >(
        ...
        let Some((exit_index_offset, _)) =
            ...
            debug_assert!(false, "There must be a TS exit after the TS entrance!");
            return false;
        };
        let mut exit_index = compact_index + exit_index_offset;

        // Assert that no flanks are involved.
        debug_assert!(self.alignment.get(exit_index + 1).is_none_or(
            |(_, alignment_type)| !matches!(
                alignment_type,
                AlignmentType::PrimaryFlankDeletion
                    | AlignmentType::PrimaryFlankInsertion
                    | AlignmentType::PrimaryFlankSubstitution
